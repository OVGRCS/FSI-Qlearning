...
def greedy(state):
    if max(Q[state] > 0):
        ...
        return actions_map[indice]
    return getRndAction(state)

#Cambiamos el valor del if segun el % o epsilon que queramos
def egreedy(state):
...
for i in xrange(Episodios):
    ...
    print "Numero de movimientos: ", (i, movimientos)
    list.append(movimientos)
    movimientos = 0
print media/Episodios
#print Q
